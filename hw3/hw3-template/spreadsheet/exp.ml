...
let mult x y = Times(ref ((get_value x) * (get_value y)), x, y)
  ...
  let rec compute_column m f =
    let rec create_lists m l m' = match m with
    | [] -> [l] @ [(compute_column m' f)]
    | h_l::tail -> match h_l with
      | [] -> []
      | hd::tl -> create_lists tail (l @ [hd]) (m' @ [tl]) 
    in List.map f (create_lists m [] [])
  ...
  let rec update e = match e with
	| Num(x) -> ()
	| Plus(v, e1, e2) -> update e1 ; update e2 ; v := (get_value e1) + (get_value e2)
	| Times(v, e1, e2) -> update e1 ; update e2 ; v := (get_value e1) * (get_value e2)
