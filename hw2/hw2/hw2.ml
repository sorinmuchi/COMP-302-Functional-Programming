...
let create_tree l = 
  let insert_2 t x = insert x t 
    in List.fold_left insert_2 Empty l
...
let rec tree_map f t = match t with
  | Empty -> Empty
  | Node((x, d), l, r) -> Node((f (x, d)), tree_map f l, tree_map f r)
...
let delete_data t =  tree_map (fun (x, d) -> x) t;;
...
let size tr = tree_fold (fun (x, s_l, s_r) -> 1 + s_l + s_r) 0  tr
let reflect tr = tree_fold (fun (x, l, r) -> Node(x, r, l)) Empty tr
let inorder  tr = tree_fold (fun(x, l, r) ->  (l @ [x]) @ r) [] tr
...
let pow_series c l = 
 fun x -> let rec rec_pow c l k = match l with
    | [] -> 0
    | a :: l -> (a * (pow (x - c) k) + rec_pow c l (k + 1))
  in rec_pow c l 0 
